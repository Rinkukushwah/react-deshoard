...
export default class Register extends Component {
  ...
  componentDidMount(){
    ...
    axios.get(`http://localhost:3001/api/v1/users/${edit_url}?token=${localStorage.token}`)
    .then(user=>{
  ...
  onSubmit(event) {
    ...
    axios.post(`http://localhost:3001/api/v1/user_update/${this.props.match.params["id"]}`, user, {
    headers: headers
    })
    .then(user => {
